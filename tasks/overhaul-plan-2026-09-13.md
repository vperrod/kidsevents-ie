...
- **Blacknight (resolved 09-13 16:50 UTC)**: the third password Victor sent works on cp.blacknighthosting.com (WHMCS client area; the old cp.blacknight.com panel rejects it and is not where the domains live). Every login needs a two-factor code delivered by email/authenticator; Victor pasted it. The vault item is updated with the working password. Account holds kinklink.ie, trystful.com, kinklinkuk.com (disabled) and smalldays.ie. Session driven from the mini PC's Chrome over CDP (reCAPTCHA blocks headless logins from the VM).
- **smalldays.ie is "Pending Registration"**: registered 2026-09-10, invoice 623359 (€7.37) **paid**, but the domain page says "This domain is not currently active. Domains cannot be managed unless active" and shows an **"Upload a Document for smalldays.ie"** box, and the DNS Manager says "You cannot edit this DNS zone because the order has not been activated". That is the IEDR proof-of-connection-to-Ireland check: **Victor uploads an ID/address document on that page** (only he can), Blacknight/IEDR activates the registration, the registry delegates the domain, and the DNS zone unlocks. Until then no DNS record can be set and nothing can reach the domain.
