...
- **Blacknight**: the vault item "Blacknight control panel (kinklink.ie DNS)" AND the password Victor sent on 09-13 15:00 are both rejected by both panels (cp.blacknight.com "Invalid login"; cp.blacknighthosting.com, a WHMCS client area, "Login Details Incorrect"), including from the mini PC's real Chrome on the home connection with no captcha in play. Three attempts per panel in total, then stopped (lockout risk). The old panel asks for a *username* (its reset link is "Forgot username or password?"), so the account's login there is probably not the email. → Victor confirms which panel holds smalldays.ie and the exact username for it (or logs in once himself and reads it from the account page); no further attempts until then.
